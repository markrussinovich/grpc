          ...
          case grpc_shmem::FrameType::C2S_MESSAGE: {
            ...
            // Final fragment: act on the full message
            {
              ...
              if (full == "cancel" && !stream_trailing_sent_[hdr.stream_id]) {
                ...
                // Echo the message back, with fast path for zero-payload
                if (full.empty()) {
                  // Zero-payload optimization: send empty message frame directly
                  ...
                  // Non-empty payload: echo back, fragmenting as needed
                  size_t offset = 0;
                 ...
                 if (!self->reader_started_.exchange(true, std::memory_order_acq_rel)) {
                   ...
                   self->client_reader_ = std::thread([self, reader_ctrl]() mutable {
                     ...
                     while (!self->stop_reader_.load(std::memory_order_relaxed)) {
                       ...
                       bool needs_flush = false;  // Batch ExecCtx flushes
                       try {
             ...
             if (!handler) {
               ...
               handler->SpawnInfallible(
                 ...
                 [h = *handler, payload = std::move(payload)]() mutable {
                                 // Zero-payload optimization: avoid SliceBuffer allocation for empty messages
                                 if (payload.empty()) {
                                   auto msg = Arena::MakePooled<Message>(SliceBuffer(), 0);
                       ...
                       // Batch flush - only call if we spawned any promises this iteration
                       if (needs_flush) {
                         ExecCtx::Get()->Flush();
                       }
                     }
